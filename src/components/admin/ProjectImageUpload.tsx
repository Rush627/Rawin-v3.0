...
import { uploadProjectPreviewAction, removeProjectPreviewAction } from "@/app/saint-denis/projects/actions";
...
export default function ProjectImageUpload({
  ...
  const activePreview = previewBlob || (!removeMarked ? imageUrl : null);
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // If existing project, trigger server action immediately
    if (projectId) {
      ...
      startTransition(async () => {
        try {
          const res = await uploadProjectPreviewAction(projectId, formData);
          if (res.error) {
            ...
            setStatusMessage({
              type: "success",
              text: "Preview image uploaded and saved successfully.",
            });
          }
        } catch {
          setStatusMessage({
      ...
      // New project: generate local blob preview; form submit will upload the binary
      const blobUrl = URL.createObjectURL(file);
      setPreviewBlob(blobUrl);
      setStatusMessage({
        type: "success",
        text: "Image selected. It will be uploaded when you save this project.",
      });
    }
  };
  ...
  const handleRemove = () => {
    ...
    if (projectId && imageUrl) {
      startTransition(async () => {
        try {
          const res = await removeProjectPreviewAction(projectId);
          if (res.error) {
            setStatusMessage({ type: "error", text: res.error });
          } else {
            setImageUrl("");
            ...
            setStatusMessage({
              type: "success",
              text: "Preview image removed.",
            });
          }
        } catch {
          setStatusMessage({
            type: "error",
            text: "Failed to remove preview image. Please try again.",
          });
        }
      });
    } else {
  ...
  return (
    <div className="flex flex-col gap-3 pt-2">
      ...
      <input type="hidden" name="previewImage" value={activePreview ? imageUrl : ""} />
      {removeMarked && <input type="hidden" name="removePreview" value="true" />}
          ...
          <div className="relative w-full aspect-[21/9] sm:aspect-[2.4/1] max-h-56 bg-ink-black/90 flex items-center justify-center overflow-hidden">
            ...
            {isPending && (
              <div className="absolute inset-0 bg-ink-black/75 backdrop-blur-xs flex flex-col items-center justify-center gap-2 text-pacific-cyan z-10">
                ...
                <span className="text-xs font-mono">Processing preview image...</span>
              </div>
            )}
          </div>
          ...
          <div className="p-3.5 flex flex-wrap items-center justify-between gap-3 bg-white/[0.02] border-t border-white/[0.08]">
            <div className="flex items-center gap-2 min-w-0">
              ...
              <span className="text-xs font-mono text-foreground truncate max-w-[200px] sm:max-w-xs">
                {filename || (imageUrl ? "Preview active" : "Selected image")}
              </span>
            </div>
