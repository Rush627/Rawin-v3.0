...
export default function FaultyTerminal({
  ...
  useEffect(() => {
    ...
    try {
      ...
      function resize() {
        ...
        const currentGl = renderer.gl;
        const currentCanvas = currentGl.canvas as HTMLCanvasElement;
      ...
      const update = (t: number) => {
        ...
        if (pageLoadAnimation && loadAnimationStartRef.current > 0 && program) {
      ...
      return () => {
        ...
        if (canvas && canvas.parentElement === ctn) {
  ...
  if (hasWebGLError) {
    return (
      <div
        className={`faulty-terminal-container bg-[#101019] ${className}`}
        style={style}
        aria-hidden="true"
        {...rest}
      />
